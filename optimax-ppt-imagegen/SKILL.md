...
1. Read [references/prompt-template.md](references/prompt-template.md) completely. If the source contains technical metaphors, literal translations, or internal system language, also read [references/chinese-copy-guidelines.md](references/chinese-copy-guidelines.md) completely.
2. Extract the page topic, one core conclusion, 3–7 main modules, their relationships, short labels, and the intended role of every accent color.
3. Unless the user explicitly requires verbatim wording, rewrite labels into natural executive Chinese before generation. Preserve meaning and facts, but replace internal or translated phrases such as “更新门”, “写后验证”, or “优先级抑制” with clear action or outcome labels. Treat the rewritten label set as the exact text for ImageGen.
4. Choose one primary color-encoding mode before choosing colors:
...
6. Treat the selected image as a style, component-language, density, and composition reference—not an edit target or content source.
7. Fill the variables in the prompt template. Do not invent metrics, customers, outcomes, or product maturity.
8. Use the installed `imagegen` skill and built-in image generation tool. Pass the selected asset as a reference image. Generate one distinct image per call.
9. Save project-bound output to the user's workspace with a descriptive, versioned PNG filename. Do not overwrite existing assets unless explicitly requested.
10. Inspect the result. Iterate with one targeted change if any acceptance check fails.
...
- Prefer natural, audience-facing Chinese labels of 4–8 characters. Avoid literal translations, unexplained internal jargon, and abstract mechanism names when a clear action or outcome label is available.
