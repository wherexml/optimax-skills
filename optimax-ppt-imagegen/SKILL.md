...
2. Extract the page topic, one core conclusion, 3–7 main modules, their relationships, exact short labels, explicitly verified success items, and explicitly pending, approval, warning, or risk items.
...
- Use navy and cobalt for every status-neutral architecture, process, gate, output, and capability node.
- Use green only when the content explicitly says an item is completed, passed, verified, or successful. Apply it as a small status badge, check, or accent—not as the full card, stage heading, label, or icon.
- Never infer green from words such as gate, publish, result, value, reusable, feedback, or future step.
- Use orange only for an explicitly pending approval, warning, risk, reminder, or adverse consequence, also as a small accent rather than a full module color.
- If the diagram describes a workflow or architecture rather than live statuses, default to blue-only. Reference-image colors never override these semantic rules.
- Pair every status color with a short label or status icon so color is never the only cue.
